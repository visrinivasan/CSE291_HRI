...
/* Blob callback*/
  void blobsCallBack (const cmvision::Blobs& blobsIn){
		...
		//              std::cout<<":: right\n";
			...
			else if(blobsIn.blobs[0].y > 380 && blobsIn.blobs[0].x>280 && blobsIn.blobs[0].x<360){
				state=3;
			}
			}
		}
...
//        std::cout<<"blob detected";   
        if(blobsIn.blobs[0].y<380 || blobsIn.blobs[0].x < 280){
//		std::cout<<":: left\n";
                geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
		cmd->linear.x=0.0;
                cmd->angular.z = 0.4;
        ...
        else if(blobsIn.blobs[0].y<380 || blobsIn.blobs[0].x > 360){
//		std::cout<<":: right\n";
                geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
                cmd->linear.x=0.0;
		cmd->angular.z = -0.4;
        ...
        if(blobsIn.blobs[0].y > 380 && blobsIn.blobs[0].x>280 && blobsIn.blobs[0].x<360){
		if(bumpflag==0){
                geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
                cmd->linear.x = 0.3;
                cmdpub_.publish(cmd);
...
int main(int argc, char **argv)
{
   ...
   */
  ros::init(argc, argv, "ball_pass_node");
